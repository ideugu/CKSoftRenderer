...
{
    {Vector2(0.0f, -0.5f), Vector3(1.0f, 0.0f, 0.0f)},
    {Vector2(0.5f, 0.5f), Vector3(0.0f, 1.0f, 0.0f)},
    {Vector2(-0.5f, 0.5f), Vector3(0.0f, 0.0f, 1.0f)}
};

bool VulkanRHI::Init(const ScreenPoint& InScreenSize)
{
...
void VulkanRHI::Clear(const LinearColor & InClearColor)
{
...
void VulkanRHI::PushStatisticText(std::string && InText)
{

}

void VulkanRHI::PushStatisticTexts(std::vector<std::string> && InTexts)
{
...
void VulkanRHI::CreateSurface()
{
    ...
    sci.hwnd = ::GetActiveWindow();

    err = vkCreateWin32SurfaceKHR(instance, &sci, nullptr, &surface);
    if (err)
    {
        throw std::runtime_error("Win32: Failed to create Vulkan surface");
    }
}
...
VkExtent2D VulkanRHI::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) 
{
    if (capabilities.currentExtent.width != UINT32_MAX) {
        ...
        VkExtent2D actualExtent = {
            static_cast<uint32_t>(_ScreenSize.X),
            static_cast<uint32_t>(_ScreenSize.Y)
        };
...
void VulkanRHI::RecreateSwapChain(const ScreenPoint& InScreenSize) {
    _ScreenSize = InScreenSize;
...
UINT32 VulkanRHI::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) 
{
